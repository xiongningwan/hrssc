...
public class FundsActivity extends BaseActivity {
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.gjjcx_btn:
                ...
                startActivity(new Intent(this, FundsDetailActivity.class));
                break;
    ...
    class BannerAsyncTask extends BaseAsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            ...
            try {
                mBanners = engine.getBanner(FundsActivity.this, token, "2");
        ...
        protected void onPostExecute(Void result) {
            if (checkException(FundsActivity.this)) {
                return;
            }
            if (mBanners != null) {
                if (mBanners.getImage() != null) {
                    ImageLoaderUtil.loadImage(mImageView, ConstantValue.FILE_SERVER_URI + mBanners.getImage(), R.mipmap.banner_gjj);
                }
            }
    ...
    class Category2AsyncTask extends BaseAsyncTask<Void, Void, Void> {
        ...
        protected Void doInBackground(Void... params) {
            IBizEngine engine = EngineFactory.get(IBizEngine.class);
            try {
                list = engine.getCategory2(FundsActivity.this, token, cid, city);
            } catch (NetException e) {
                exception = e;
                e.printStackTrace();
            }
